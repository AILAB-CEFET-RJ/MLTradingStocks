...
for file in filenames:
    ...
    for i in tds_last_10_times:
        try:
            ...
            # day = [datetime.strptime(date, '%Y-%m-%d') for x in range(10)]
    ...
    filepath = os.getcwd() + '/consolidado.csv'
    # print(filepath)

    if os.path.exists(filepath):
        dfOld = pd.read_csv('consolidado.csv')
        dfNew = dfOld.append(final_array_pd, ignore_index=False)
        dfNew.to_csv('consolidado.csv', index=False, encoding='utf-8')
    else:
        final_array_pd.to_csv('consolidado.csv', index=False, encoding='utf-8')
