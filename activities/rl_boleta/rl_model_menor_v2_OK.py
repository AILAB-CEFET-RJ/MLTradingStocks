...
class ReinforcementLearningEnv(gym.Env):
    ...
    def step(self, action):
        ...
        carry_action_reward = CarryAndActionReward(self.passo_atual_total, self.max_steps, self.current_observation_price, self.last_observation_price, self.shares_held, self.action_type, self.quantidade_executada)
        reward_step = carry_action_reward.calculate_reward()

        self.recompensas_por_acao_episodio.append(reward_step)

        done = (self.net_worth <= (self.initial_amount * self.stop_condition) ) or (self.deslocamento == (len(self.df)) and self.mode == 'training')
