...
import pdb
import math
from reward_zone import reward_zone_1, reward_zone_2, reward_zone_net_worth
...
class ReinforcementLearningEnv(gym.Env):
    ...
    def __init__(self, mode, df, initial_amount=10000.00, stop_condition=0.8, episodios=[], 
    ...
    def step(self, action):
        ...
        # reward_step = reward_zone_1(self.passo_atual_total, self.max_steps, self.current_observation_price, 
        #               self.last_observation_price, self.shares_held, self.action_type, self.quantidade_executada)
        reward_step = reward_zone_net_worth(self.passo_atual_total, self.max_steps, self.net_worth)

        self.recompensas_por_acao_episodio.append(reward_step)

        done = (self.net_worth <= (self.initial_amount * self.stop_condition) ) or (self.deslocamento == (len(self.df) - 10) and self.mode == 'training')
    ...
    def _take_action(self, action):
        ...
        if math.isclose(amount_bought_or_sold, 0.0) or self.action_type == 0:
            ...
            if 0 < self.action_type <= 1:
                ...
                self.acoes_vendidas.append(0)
                if shares_bought == 0:
                    self.sem_recursos = True

            # VENDA - Sell amount % of shares held
            elif -1 <= self.action_type < 0:
